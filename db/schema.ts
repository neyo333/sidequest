...
export const userSettings = pgTable("user_settings", {
  ...
  enabledDefaultQuests: jsonb("enabled_default_quests").$type<string[]>().default([]).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const dailyQuestsRelations = relations(dailyQuests, ({ one }) => ({
  ...
  quest: one(quests, { fields: [dailyQuests.questId], references: [quests.id] }),
}));
...
export const insertUserSettingsSchema = createInsertSchema(userSettings).omit({ userId: true, createdAt: true, updatedAt: true });
...
export type UpdateQuestRequest = { id: number; content: string };
export type QuestResponse = Quest;
export type DailyQuestResponse = DailyQuest;
export type DayStats = { date: string; count: number; completedQuests: DailyQuest[] };
...
// === DEFAULT QUESTS ===
// 📍 MODIFY THESE QUESTS HERE:
export interface DefaultQuest { id: string; content: string; category: string; }
...
export const ACHIEVEMENT_TYPES = {
  ...
  STREAK_500: "streak_500", // Changed from 400 to 500
} as const;
